...
3. Build the palette from the complete provided logo: `primary` is the main prominent logo colour; `secondary` is another suitable logo colour and may contrast strongly with Primary; `accent` is another suitable logo colour when available; and `text` is `#000000`. Black is a valid Secondary. Use additional logo colours as supporting/custom globals where useful. Derive shades or tonal variations only when the logo does not provide enough usable colours. Never invent a hue absent from the logo unless the user explicitly supplies a palette override.
